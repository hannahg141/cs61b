...
public class MatrixUtils {
    ...
    public static double[][] accumulateVertical(double[][] m) {
        ...
        for (int r = 1; r < height; r++) {
            for (int c = 0; c < width; c++) {
                ...
                left = helpGet(am, r - 1, c - 1);
                middle = helpGet(am, r - 1, c);
                right = helpGet(am, r - 1, c + 1);
                if (left < lowest) {
                    lowest = left;
                }
                if (middle < lowest) {
    ...
    /** helping get the valid return of our matrix.
     * @return the matrix @param m
     * @param r ** this used for row **
     * @param c ** this used for column**
     *  assuming that they exist in our matrix
     *  @param m ** matrix **
     *  */
    private static double helpGet(double[][] m, int r, int c) {
        int width = m[0].length;
        int height = m.length;
    ...
    public static double[][] accumulate(double[][] m, Orientation orientation) {
        double[][] mine = MatrixUtils.copy(m);
        if (orientation == Orientation.HORIZONTAL) {
            mine = makeVertical(mine);
        }
        return makeVertical(accumulateVertical(mine));
    }

    /** this turns a horizontal matrix into a vertical one.
     *
     * @return mt the new matrix-- vertical enabled
     * @param m **initial matrix**
     * */
    public static double[][] makeVertical(double[][] m) {
        int height = m.length;
        int width = m[0].length;
