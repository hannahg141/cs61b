...
public class Graph {
    ...
    public int[] dijkstras(int startVertex) {
        ...
        Queue<Integer> fringe = new PriorityQueue<>((e1, e2) -> (result[e1] - result[e2]));
        ...
        while(!fringe.isEmpty()) {
            ...
            seen.add(curr);
            for (Integer neighbor : neighbors(curr)) {
                if (isAdjacent(curr, neighbor)
                        && (result[neighbor] > result[curr] + getEdge(neighbor, curr).edgeWeight)) {
                    result[neighbor] = result[curr] + getEdge(neighbor, curr).edgeWeight;
                }
                fringe.add(neighbor);
            }
