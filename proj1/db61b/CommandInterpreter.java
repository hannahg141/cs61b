...
class CommandInterpreter {
    ...
    void insertStatement() {
        ...
        while (_input.nextIf(",")) {
            _input.next("(");
            int cols2 = table.columns();
            String[] values2 = new String[cols2];
            values2[0] = literal();
            for (int i = 1; _input.nextIf(","); i++) {
                values2[i] = literal();
            }
    ...
    void loadStatement() {
        ...
        Table myTable = Table.readTable(tableName);
        _database.put(tableName, myTable);
        _input.next(";");
    ...
    void printStatement() {
        _input.next("print");
        String tabName = _input.peek();
        Table table = tableName();
        System.out.print("Contents of " + tabName + ": \n");
        table.print();
        _input.next(";");
    ...
    Table selectClause() {
        ...
        _input.next("from");
        Table temp1 = tableName();
        if (_input.nextIf(",")) {
            Table temp2 = tableName();
            conditions = conditionClause(temp1, temp2);
            finalTable = temp1.select(temp2, columnNames, conditions);
        } else {
            conditions = conditionClause(temp1);
            finalTable = temp1.select(columnNames, conditions);
        }
    ...
    ArrayList<Condition> conditionClause(Table... tables) {
        ...
        if (_input.nextIf("where")) {
            Condition condTemp = condition(tables);
            myConditions.add(condTemp);
            while (_input.nextIf("and")) {
                Condition condTemp2 = condition(tables);
                myConditions.add(condTemp2);
            }
        }
    ...
    Condition condition(Table... tables) {
        ...
        column1 = new Column(columnName(), tables);
        ...
        if (_input.nextIs(Tokenizer.LITERAL)) {
            ...
            column2 = new Column(columnName(), tables);
