...
class Move {
    ...
    static Move move(Move move0, Move move1) {
        ...
        if (move0.isVestigial()) {
            return move(move(move0._col0, move0._row0,
        ...
        if (move0.jumpTail() == null) {
            ...
            return move(move0._col0, move0._row0, move0._col1,
                    move0._row1, move(move0.jumpTail(), move1));
        }
    ...
    /**is position K able to move diagonally.
     * @param c col
     * @param r row
     * @return if I can move diagonally.
     */
    boolean isDiagonal(char c, char r) {
        return isDiagonal(index(c, r));
    }

    /**Can position K go diagonally.
     * @param k
     * @return bool.
     */
    boolean isDiagonal(int k) {
        if (k % 2 == 0) {
            return true;
        }
        return false;
    ...
    char jumpedRow() {
        ...
        if (isJump()) {
            if (col0() == col1() || isDiagonal(col0(), row0())) {
                if (row0() < row1()) {
                    return (char) (row0() + 1);
                } else {
                    return (char) (row0() - 1);
    ...
    char jumpedCol() {
        ...
        if (isJump()) {
            if (row0() == row1() || isDiagonal(col0(), row0())) {
                if (col0() < col1()) {
                    return (char) (col0() + 1);
                } else {
                    return (char) (col0() - 1);
    ...
    /** Return the linearized index of (jumpedCol(), jumpe
     *dRow()). */
    int jumpedIndex() {
    ...
    private void toString(Formatter out) {
        ...
        Move jumpNow = _nextJump;
        char col = col1();
        char row = row1();
        while (jumpNow != null && !jumpNow.isVestigial()) {
            if (jumpNow.col0() == col && jumpNow.row0() == row) {
                out.format("-%c%c", jumpNow.col1(), jumpNow.row1());
            } else {
                out.format("-%c%c-%c%c", jumpNow.col0(),
                        jumpNow.row0(), jumpNow.col1(), jumpNow.row1());
            }
            col = jumpNow.col1();
            row = jumpNow.row1();
            jumpNow = jumpNow._nextJump;
        }
